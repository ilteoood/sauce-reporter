...
describe('previousMonthRange', () => {
  it('returns the first day of last month to the end of the last day of last month', () => {
    ...
    assert.strictEqual(range.from.toISOString(), '2026-05-01T00:00:00.000Z');
    assert.strictEqual(range.to.toISOString(), '2026-05-31T23:59:59.999Z');
  });

  it('handles year boundary: January rolls back to December of previous year', () => {
    const now = new Date('2026-01-05T00:00:00Z');
    const range = previousMonthRange(now);
    assert.strictEqual(range.from.toISOString(), '2025-12-01T00:00:00.000Z');
    assert.strictEqual(range.to.toISOString(), '2025-12-31T23:59:59.999Z');
  });

  it('handles leap year February with twenty-nine days', () => {
    const now = new Date('2024-03-15T00:00:00Z');
    const range = previousMonthRange(now);
    assert.strictEqual(range.from.toISOString(), '2024-02-01T00:00:00.000Z');
    assert.strictEqual(range.to.toISOString(), '2024-02-29T23:59:59.999Z');
  });

  it('handles March 1 boundary where February is non-leap', () => {
    const now = new Date('2025-03-01T00:00:00Z');
    const range = previousMonthRange(now);
    assert.strictEqual(range.from.toISOString(), '2025-02-01T00:00:00.000Z');
    assert.strictEqual(range.to.toISOString(), '2025-02-28T23:59:59.999Z');
  });
});
...
describe('resolveDateRange', () => {
  ...
  it('parses explicit from/to inputs and keeps the to date inclusive', () => {
    ...
    assert.strictEqual(range.from.toISOString(), '2026-01-01T00:00:00.000Z');
    assert.strictEqual(range.to.toISOString(), '2026-01-31T23:59:59.999Z');
  });

  it('accepts a single-day range when to equals from', () => {
    const range = resolveDateRange('2026-07-15', '2026-07-15');
    assert.strictEqual(range.from.toISOString(), '2026-07-15T00:00:00.000Z');
    assert.strictEqual(range.to.toISOString(), '2026-07-15T23:59:59.999Z');
  });
