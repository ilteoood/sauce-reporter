...
describe('previousMonthRange', () => {
  it('returns the first day of last month to the end of the last day of last month', () => {
    assert.deepStrictEqual(previousMonthRange(new Date('2026-06-15T12:34:56Z')), {
  ...
  it('handles year boundary: January rolls back to December of previous year', () => {
    assert.deepStrictEqual(previousMonthRange(new Date('2026-01-05T00:00:00Z')), {
  ...
  it('handles leap year February with twenty-nine days', () => {
    assert.deepStrictEqual(previousMonthRange(new Date('2024-03-15T00:00:00Z')), {
  ...
  it('handles March 1 boundary where February is non-leap', () => {
    assert.deepStrictEqual(previousMonthRange(new Date('2025-03-01T00:00:00Z')), {
      from: new Date('2025-02-01T00:00:00.000Z'),
      to: new Date('2025-02-28T23:59:59.999Z'),
    });
  });
});
...
describe('resolveDateRange', () => {
  it('falls back to previous month when inputs are empty', () => {
    assert.deepStrictEqual(resolveDateRange('', '', new Date('2026-06-15T00:00:00Z')), {
      from: new Date('2026-05-01T00:00:00.000Z'),
      to: new Date('2026-05-31T23:59:59.999Z'),
    });
  });
  ...
  it('parses explicit from/to inputs and keeps the to date inclusive', () => {
    assert.deepStrictEqual(resolveDateRange('2026-01-01', '2026-01-31'), {
  ...
  it('accepts a single-day range when to equals from', () => {
    assert.deepStrictEqual(resolveDateRange('2026-07-15', '2026-07-15'), {
